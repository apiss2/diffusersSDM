...
class SDMDataset(Dataset):
    ...
    def __getitem__(self, i: int):
        ...
        # generate one-hot label: (C,H,W)
        shape = (self.num_classes, mask.shape[0], mask.shape[1])
        onehot_label = np.zeros(shape, dtype=np.float32)
        valid = (mask >= 0) & (mask < self.num_classes)
        zidx, yidx, xidx = mask[valid], np.where(valid)[0], np.where(valid)[1]
        onehot_label[zidx, yidx, xidx] = 1.0
        seg = torch.from_numpy(onehot_label)

        return {"image": img, "semantic_label": seg}
