...
class ResBlock(TimestepBlock):
    """Residual block that can change number of channels."""
    ...
    def __init__(
        ...
        self.in_layers = nn.Sequential(
            normalization(channels), SiLU(), conv_nd(dims, channels, self.out_channels, 3, padding=1)
        )
...
class SDMResBlock(CondTimestepBlock):
    """Residual block conditioned on segmentation map."""
    ...
    def __init__(
        ...
        self.in_layers = nn.Sequential(
            SiLU(), conv_nd(dims, channels, self.out_channels, 3, padding=1)
        )
...
class QKVAttention(nn.Module):
    """QKV attention with different split order."""
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.time_embed = nn.Sequential(
            linear(model_channels, time_embed_dim), SiLU(), linear(time_embed_dim, time_embed_dim)
        )
        ...
        self.out = nn.Sequential(
            normalization(ch), SiLU(), zero_module(conv_nd(dims, input_ch, out_channels, 3, padding=1))
        )
    ...
    def forward(self, x, timesteps, y=None):
        ...
        y = y.type(self.dtype) if y is not None else None
        h = x.type(self.dtype)
...
class EncoderUNetModel(nn.Module):
    """Half UNet model with attention and timestep embedding."""
